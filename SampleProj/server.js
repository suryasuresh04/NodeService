...
app.use(bodyParser.json());
 ...
 var funcBing = function(audio,lastObj){
	...
	return new Promise(function(resolve, reject) {
	 ...
	 if(audio)
	 {
		 audioStream = fs.createReadStream('http://deltagoservice.azurewebsites.net/uploads/test.wav'); 
	 }
	 else{
 ...
 var funcText = function(audio,lastObj){
	...
	var client = new bing.BingSpeechClient(subscriptionKey);*/
	console.log("inside functext");
	console.log(audio);
	 funcLUISText(audio,lastObj)
			.then(response => deltaApiCall(response,lastObj))
			.then(response => resolve(response));
	});
 }
...
console.log(body);
 ...
 var deltaApiCall = function(data,lastObj){
	return new Promise(function(resolve, reject) {
		...
		{
			data.flightNumber = "Hi User Good Day, what can I do for you?";
			resolve(data);
		}
 ...
 var funcLUISText =  function(response,lastObj){
	 return new Promise(function(resolve, reject) {
	...
	if(response)
	{
	var returnResponse = LUISclient.predict(response, {
...
app.post('/profile', upload.single('https://deltagoservice.azurewebsites.net/test.wav'), function (req, res, next) {
...
app.post('/profileText', function (req, res, next) {
	console.log("inside profile text");
  // req.file is the `avatar` file
  // req.body will hold the text fields, if there were any
  ...
  //console.log(req.param.uptext);
  console.log(req.body);
  console.log("inside profile text");
  funcText(req.body.uptext,req.session.lastObj).then(response => {req.session.lastObj = response; res.send(response)});
  // res.send("success");
})
